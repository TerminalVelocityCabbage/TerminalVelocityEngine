...
public class ModelMeshPart extends MeshPart {
	...
	private final VertexXYZNormalUV[] vertices;
	private final short[] vertexOrder;

	private int vertexOffset = -1;

	public ModelMeshPart(VertexXYZNormalUV[] vertices, short[] vertexOrder) {
	...
	public VertexXYZNormalUV getVertex(int num) {
		return vertices[num];
	}
	...
	public void update(Matrix4f translationMatrix, FloatBuffer buffer) {
		...
		float[] currentXYZ;
		float[] currentNormal;
		buffer.position(this.vertexOffset * VertexXYZNormalUV.ELEMENT_COUNT);
		for (VertexXYZNormalUV currentVertex : vertices) {
			...
			buffer.put(VertexXYZNormalUV.getElements(positions.x, positions.y, positions.z, currentVertex.getUV(), normals.x, normals.y, normals.z));
		}
	}
}
